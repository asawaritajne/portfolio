      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(460px,1fr))", gap: 24, alignItems: "start" }}>
        {PROJ.map((p, i) => (
          <Reveal key={i} delay={i * .1}>
            <div style={{ background: "rgba(255,255,255,.022)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 22, overflow: "hidden", transition: "all .4s cubic-bezier(.22,1,.36,1)", display: "flex", flexDirection: "column", height: "100%" }}
            ...
            >
              ...
              <div style={{ padding: "0 28px 28px", display: "flex", flexDirection: "column", flex: 1 }}>
                ...
                <a href={p.url} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "11px 24px", border: `1px solid ${p.color}55`, borderRadius: 50, fontFamily: "'Outfit',sans-serif", fontSize: 14, fontWeight: 600, color: p.color, transition: "all .3s", cursor: "pointer", background: `${p.color}00` }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: 24, alignItems: "stretch" }}>
        {EDU.map((e, i) => (
          <Reveal key={i} delay={i * .14}>
            <div style={{ position: "relative", background: "rgba(255,255,255,.022)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 22, overflow: "hidden", transition: "all .4s", height: "100%" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, maxWidth: 860, margin: "0 auto" }}>
        {CON.map((c, i) => (
          <Reveal key={i} delay={i * .08}>
            <a href={c.href} target="_blank" rel="noopener noreferrer" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "32px 20px", background: "rgba(255,255,255,.022)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 20, transition: "all .35s cubic-bezier(.22,1,.36,1)", cursor: "pointer", textDecoration: "none" }}
            ...
            >
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 18, color: c.color, width: 52, height: 52, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 14, background: `${c.color}12`, border: `1px solid ${c.color}25`, boxShadow: `0 0 20px ${c.color}18` }}>{c.icon}</span>
              <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: 14, color: "#fff" }}>{c.label}</span>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, color: "#555", textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%", display: "block" }}>{c.val}</span>
            </a>
          </Reveal>
        ))}
      </div>
